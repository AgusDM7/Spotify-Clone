...
export function PlayerControlButtonBar() {
  ...
  return (
    <div className="flex justify-center flex-row flex-nowrap items-center gap-7">
      <button className="text-neutral-400 hover:scale-110" onClick={onPrevSong} title="Previous song">
        <Prev/>
      </button>
      <button className="bg-white text-black rounded-full p-2 hover:scale-110" onClick={onPlayPause}>
        {isPlaying ? <Pause/> : <Play/>}
      </button>
      <button className="text-neutral-400 hover:scale-110" onClick={onNextSong} title="Next song">
        <Next/>
      </button>
    </div>
  );
